...
class NavbarStateNotifier extends StateNotifier<NavbarState> {
  ...
  void initPositions({required int itemCount, required double containerWidth}) {
    ...
    state = state.copyWith(
      positions: positions,
      draggablePosition: positions[state.currentIndex < itemCount ? state.currentIndex : 0],
    );
  }
  ...
  void setCurrentIndex(int index) {
    if (state.positions.isEmpty || index >= state.positions.length) return;
...
final liquidNavbarStateProvider =
